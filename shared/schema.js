...
const recipeTags = pgTable("recipe_tags", {
  id: serial("id").primaryKey(),
...
const mealPlanItems = pgTable("meal_plan_items", {
  id: serial("id").primaryKey(),
...
const recipeTagsRelations = relations(recipeTags, ({ one }) => ({
...
const mealPlanItemsRelations = relations(mealPlanItems, ({ one }) => ({
...
const insertUserSchema = createInsertSchema(users).omit({ createdAt: true, updatedAt: true });
const insertCategorySchema = createInsertSchema(categories).omit({ id: true });
const insertTagSchema = createInsertSchema(tags).omit({ id: true });
const insertRecipeSchema = createInsertSchema(recipes).omit({ id: true, createdAt: true, updatedAt: true, viewCount: true });
const insertFavoriteSchema = createInsertSchema(favorites).omit({ id: true, createdAt: true });
const insertRatingSchema = createInsertSchema(ratings).omit({ id: true, createdAt: true });
const insertCommentSchema = createInsertSchema(comments).omit({ id: true, createdAt: true, updatedAt: true });
const insertMealPlanSchema = createInsertSchema(mealPlans).omit({ id: true, createdAt: true });
const insertMealPlanItemSchema = createInsertSchema(mealPlanItems).omit({ id: true });
export {
  categories,
  categoriesRelations,
  comments,
